...
- Local verification: 34 Vitest tests, lint, typecheck, production build, route HTTP checks, real API create/retrieve/render/invalid-ID checks, and browser builder-to-generated-link flow have passed.
...
- Repository security: CodeQL default setup is enabled. It identified four high-severity polynomial-regex findings across custom and legacy email validation. Both paths now share bounded linear checks with focused adversarial regression coverage; the feature JavaScript/TypeScript, Actions, and CodeQL gates passed.
