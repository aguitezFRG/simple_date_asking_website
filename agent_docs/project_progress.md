...
- Repository security: CodeQL default setup is enabled, and the initial JavaScript/TypeScript and Actions analyses on `main` passed. The feature-head scan, integration, final main workflow, and production smoke checks remain pending and must not be reported complete.
