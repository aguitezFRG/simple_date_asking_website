...
- Repository security: CodeQL default setup is enabled, and the initial JavaScript/TypeScript and Actions analyses on `main` passed. The first feature-head scan identified two high-severity polynomial-regex findings in email validation; the regex was replaced with bounded linear checks and focused regression coverage. The fixed feature-head scan, integration, final main workflow, and production smoke checks remain pending and must not be reported complete.
