...
- Vitest/jsdom/Testing Library infrastructure with 34 passing schema, UI, route, storage, identifier, access-control, demo, default-form, and adversarial email-validation regression tests.
...
- A final main audit found the same two findings in the legacy default submission route. It now reuses the shared linear validator with focused route regression coverage while preserving its existing validation messages and SMTP behavior.
