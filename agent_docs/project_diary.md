...
- The project uses `pnpm check` as its consolidated validation command. There is no formatter or automated test suite, so validation reports must not imply either exists.
